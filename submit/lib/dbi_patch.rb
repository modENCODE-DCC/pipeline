...
class DBI::DBD::Pg::Database
    def load_type_map
        # Get type map path
        gbrowse_tmp = RsyncUploadController::gbrowse_tmp
        typemap_path = File.join(gbrowse_tmp, "type_map.marshal")
        if File.exists?(typemap_path) then
          puts "DBI_PATCH::Loading type map"
          @type_map = Marshal.restore(File.read(typemap_path))
          puts "DBI_PATCH::Done."
          return
        end
        @type_map = Hash.new
      ...
      File.new(typemap_path, "w").puts(Marshal.dump(@type_map))
