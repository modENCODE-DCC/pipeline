require 'track_finder_fast'

class FindTracksFastController < CommandController
  def initialize(options)
    ...
    logger.info "Creating new FindTracksFast object with #{options}"
    self.command_object = FindTracksFast.new(options)
  ...
  def run
    super do
      command_object.status = FindTracksFast::Status::FINDING
      command_object.stdout = ""
      command_object.stderr = ""
      command_object.save

      track_finder = TrackFinderFast.new(command_object)
      ...
      unless schemas[schema] then
        ...
        command_object.status = FindTracksFast::Status::FINDING_FAILED
        command_object.save
      ...
      command_object.status = FindTracksFast::Status::FOUND
      command_object.save
  ...
  def do_after(options = {})
    if self.status == FindTracksFast::Status::FINDING_FAILED then
      return false
    else
      return true
    end
