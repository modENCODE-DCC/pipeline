...
sub connect {
  print STDERR "ONE TWO THREE\n";
    ...
    $self->{_schemaname} = shift;
    $dbi = $self->resolve_dbi($dbi);
    eval {
	$self->dbh(DBI->connect($dbi, @_));
	$self->schema_dbh(DBI->connect($dbi, @_));
    };
    if ($@ || !$self->dbh || !$self->schema_dbh) {
...
EOM
    ...
    $self->schema_dbh->{RaiseError} = 1;
    $self->schema_dbh->{ShowErrorStatement} = 1;
    if ($dbi =~ /dbi:([\w\d]+)/) {
        $self->{_driver} = $1;
    }
    $self->setup;
...
sub schema_dbh {
    my $self = shift;
    $self->{_schema_dbh} = shift if @_;
    return $self->{_schema_dbh};
}

sub dbschema {
    my $self = shift;
    $self->{_dbschema} = shift if @_;
    my $schemaname = $self->{_schemaname};
    if (!$self->{_dbschema}) {
        if (!$self->schema_dbh) {
            confess("you must establish connection using connect() first");
        }
        $self->dbschema(DBIx::DBSchema->new_native_with_schema($schemaname, $self->schema_dbh));
