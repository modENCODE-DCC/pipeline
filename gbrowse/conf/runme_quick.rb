...
all_track_defs = TrackStanza.find_all_by_project_id(quick_project_id)
...
track_defs.reject! { |track, config| 
  ...
  stanza_organism = "fly" if config[:organism] == "Drosophila ananassae"  # should be dana
  stanza_organism = "fly" if config[:organism] == "Drosophila virilis"	   # should be dvir	
  stanza_organism = "fly" if config[:organism] == "Drosophila yakuba"	   # should be dyak
...
# Databases
if track_defs.nil? || track_defs.empty? then
  puts config_text
  exit
end
...
seen_dbs = Array.new
all_configs = track_defs.values + track_defs.values.map { |definition| 
  subdefs = (( definition[:semantic_zoom] && ! definition[:semantic_zoom].empty?) ? definition[:semantic_zoom].values : []);
#  subdefs = (definition[:semantic_zoom] ? definition[:semantic_zoom].values : []);
  subdefs.each { |subdef| subdef["data_source_id"] = definition["data_source_id"] }
  subdefs
}.flatten(1).compact
#track_defs.each do |stanzaname, definition| 
all_configs.each do |definition|
  ...
  if database =~ /^modencode_bam_/ then
    ...
    config_text << "\n"
  elsif database =~ /^modencode_bigwig_/ then
    project_id = definition["data_source_id"].split(" ").first
    bigwig_file_path = File.join(ExpandController.path_to_project_id_dir(project_id), "tracks", definition[:bigwig_file])
    config_text << "[#{database}:database]\n"
    config_text << "db_adaptor    = Bio::DB::BigWig\n"
    config_text << "db_args       = sub { require Bio::DB::Sam;\n"
    config_text << "                      return (\n"
    config_text << "                        -bigwig => '#{bigwig_file_path}',\n"
    config_text << "                        -fasta => Bio::DB::Sam::Fai->open('../bam_support_fasta/#{organism}.fa')\n"
    config_text << "                      );\n"
    config_text << "                }\n"
    config_text << "search options = none\n"
    config_text << "\n"
  else
