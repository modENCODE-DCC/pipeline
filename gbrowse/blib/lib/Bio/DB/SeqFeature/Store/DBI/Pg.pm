...
END
  ...
  $sth->execute($id,unpack('H*', $self->freeze($object)),$index_flag||0,@location,$typeid) or $self->throw($sth->errstr);
...
END
  $sth->execute(undef,unpack('H*', $self->freeze($object)),$index_flag) or $self->throw($sth->errstr);
  my $dbh = $self->dbh;
...
sub _dump_store {
  ...
  for my $obj (@_) {
    ...
    my $typeid   = $self->_typeid($primary_tag,1);
		...
		my $frozen_object = $dbh->quote(unpack('H*', $self->freeze($obj)), { pg_type => PG_BYTEA});
		$frozen_object =~ s/^E?'|'$//g;
		$frozen_object =~ s/\\\\/\\/g;
