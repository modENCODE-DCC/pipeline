class ValidateController < CommandController
  def initialize(options)
    super
    if block_given? then
      # Don't create a Validate object if a subclass gave us a block to use
      yield
      return if self.command_object
    end
    return unless options[:command].nil? # Set in CommandController if :command is given

    self.command_object = Validate.new(options)
    ...
    command_object.command = "#{validator} #{project_type_params} \"#{package_dir}\""
